...
unsigned int protocol_flag = 0;
unsigned int count = 0;

unsigned char src_mac[6] = {0x01,0x02,0x03,0x04,0x05,0x06}; //Pynq
unsigned char dst_mac[6] = {0X00,0x0C,0x29,0xb7,0x9a,0x5e}; //VMware the laptop thinkpad X1--jcyuan
...
void analyseDATA(DATA_RECEIVE *data_recv,u32*s_addr,u32*s_data,u32*s_times,u32*s_inc_dec,u32*s_cycle)
{
	*s_addr=(data_recv->addr)&0x3ff;
	*s_data=(data_recv->data)&0xff;
	*s_times=(data_recv->times)&0xffff;
	*s_inc_dec=data_recv->inc_dec;
	*s_cycle=data_recv->cycle;
}
