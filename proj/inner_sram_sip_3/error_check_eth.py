...
for i in range(min(len(l_lines),len(v_lines))):
	...
	if v_line[0]==str(WRT_ONE):
		if len(v_line)!=3:
			...
			data=int(v_line[2])
			SRAM[addr]=data
		if len(l_line)!=5 or hex_to_int(r_times)!=0 or hex_to_int(r_opcfg)!=0:
			print("ERROR LOG")
			e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
			continue
		else:
			if cmd_mode!='WRT_ONE' or hex_to_int(r_data)!=SRAM[hex_to_int(r_addr)] :
				print("ERROR LOG")
				e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
			else:
	...
	#TWO
		...
		if len(v_line)!=2:
			...
			if len(l_line)!=5 or hex_to_int(r_times)!=0 or hex_to_int(r_opcfg)!=0:
				...
				e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			if cmd_mode!='READ' or hex_to_int(r_addr)!=int(v_line[1]) :
				print("ERROR LOG")
				e_file.write("ERROR_LOG_ADDR: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			if hex_to_int(r_data)!=SRAM[hex_to_int(r_addr)]:
				print("ERROR LOG")
				e_file.write("ERROR_LOG_DATA: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+' theory: '+str(hex(SRAM[hex_to_int(r_addr)]))+'\n')
				continue
			else:
				print("CHECK ONE FINISH")
				continue
	#THREE
		...
		if len(v_line)!=2:
			...
			for j in range(ADDR_RANGE+1):
				SRAM[j]=data
		if len(l_line)!=5 or hex_to_int(r_addr)!=0 or hex_to_int(r_times)!=int('0x3ff',16) or hex_to_int(r_opcfg)!=0:
			print("ERROR LOG")
			e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
			continue
		else:
			if cmd_mode!='WRT_ALL' or data!=hex_to_int(r_data):
				print("ERROR LOG")
				e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			else:
	...
	#FOUR
		...
		if len(l_line)!=5:
			...
			if cmd_mode!='WRT_CFG' or (inc_dec*2+cycle)!=hex_to_int(r_opcfg) or hex_to_int(r_times)!=times or hex_to_int(r_addr)!=addr or hex_to_int(r_data)!=data:
				print("ERROR LOG")
				e_file.write("ERROR_LOG: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			else:
