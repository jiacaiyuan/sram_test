...
int main(int argc,char* argv[])
{
    ...
    socket_init();
    read_vectors(argv[2],argv[3],argv[4],argv[5],argv[6],argv[7],&data_send);
    //MAC
    ...
    if(protocol_flag == 1)
    {
        ...
        while(!strcmp(data_recv->cmd_mode, "UNKNOW"))
        {
           // usleep(100000);//0.1s
...
int read_vectors(char*cmd,char*addr,char*data,char*times,char*inc_dec,char*cycle,DATA_SEND*data_send)
{
    data_send->cmd=atoi(cmd);
    if (atoi(cmd)==1)//write one
    {
        data_send->addr=atoi(addr)&ADDR_RANGE;
        data_send->data=atoi(data)&DATA_RANGE;
        data_send->times=0;
        data_send->inc_dec=0;
        data_send->cycle=0;
    }
    else if(atoi(cmd)==2)//read one
    {
        data_send->addr=atoi(addr)&ADDR_RANGE;
        data_send->data=0;
        data_send->times=0;
    ...
    else if(atoi(cmd)==4)//write config
    {
        data_send->addr=atoi(addr)&ADDR_RANGE;
        data_send->data=atoi(data)&DATA_RANGE;
        data_send->times=atoi(times)&ADDR_RANGE;
        if(atoi(inc_dec)!=0){ data_send->inc_dec=1;}
        else{ data_send->inc_dec=0;}
        if(atoi(cycle)!=0){ data_send->cycle=1;}
