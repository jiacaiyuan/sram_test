...
#3 data
#4 addr
#5 addr data op_area inc_dec cycle
#6 addr area
#7 cut_connection
import os
import sys
	...
	#TWO READ 
		...
		if len(v_line)!=2:
			...
			if hex_to_int(l_line[1])!=int(v_line[1]) :
				...
				continue
			if hex_to_int(l_line[2])!=INNER[hex_to_int(l_line[1])]:
				print("ERROR LOG")
				e_file.write("ERROR_LOG_DATA: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+' theory: '+str(hex(INNER[hex_to_int(l_line[1])]))+'\n')
				continue
			else:
				print("CHECK ONE FINISH")
				continue	
	...
	#SIX update
		...
		if len(v_line)!=3:
			...
			if hex_to_int(l_line[1])!=int(v_line[1]) :
				...
				e_file.write("ERROR_LOG_ADDR: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			if hex_to_int(l_line[2])!=int(v_line[2]):
				print("ERROR LOG")
				e_file.write("ERROR_LOG_AREA: "+l_file_name+"("+str(i+1)+"): "+l_lines[i].strip()+"         "+"CMD: "+v_lines[i].strip()+'\n')
				continue
			else:
				print("CHECK ONE FINISH")
				continue
	else:
		print("NO Recognition vector file line "+str(i+1))
		print("ERROR CMD")
		continue
